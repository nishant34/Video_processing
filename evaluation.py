...
def Run_video(Fs, Ms, num_frames, num_objects, Mem_every=None, Mem_number=None):
    ...
    #std =  torch.FloatTensor([0.485, 0.456, 0.406]).view(1,3,1,1)
    Fs = Fs.cuda()
    Es = Es.cuda()
    #Fs[:,:,0] = helpers.ToCuda(Fs[:,:,0])
    #Es[:,:,0] = helpers.ToCuda(Es[:,:,0])
    for t in tqdm.tqdm(range(1, num_frames)):
        # memorize
        with torch.no_grad():
            #Fs[:,:,t-1] = (Fs[:,:,t-1] - mean)/std
...
if __name__ == '__main__' :
 ...
 VIZ = args.viz
 #DATA_ROOT = args.D
 DATA_ROOT = 'C:/Users/DELL/Desktop/RGMP/DAVIS'
